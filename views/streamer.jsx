  ...
  <div className="page | page-wrap-1600 pos-relative left-pad-22_0 right-pad-1_0 y-pad-1_0">
      ...
      <div className="profile-image | img-contain x-pad-1_0 parent">
        <div className=" | x-pad-0_5 y-pad-0_5 border-radius-0_8 rgb-bg-100_200_250">
          <img className=" | display-block border-radius-0_5" src={this.props.userData.logo} />
          ...
          <div className="channel-options | col-2">
            <div className="message">
              <a className=" | rgb-text-240_250_250 no-underline bold" href={`http://www.twitch.tv/message/compose?to=${this.props.userData.name}`} target="_blank">Message</a>
            </div>
          </div>
        </div>
      </div>
      <div className="channel-name | center-justify">
        <h3 className="top-marg-1_0">{this.props.userData.display_name}</h3>
      </div>
      <div className="separator | height-0_1 y-marg-1_0 rgb-bg-100_200_250"></div>
      <div className="channel-bio | x-pad-0_5">
        <p className=" | unset">{this.props.userData.bio}</p>
      </div>
      <div className="separator | height-0_1 y-marg-1_0 rgb-bg-100_200_250"></div>
    ...
    <div className="main-section | pos-relative full-width x-pad-1_0 y-pad-1_0 border-radius-1_0 rgb-bg-220_240_250">
      <div className="header |">
        ...
        <div className="stream-text | inline-block">
          <div className="title">
            <h1 className=" | unset font-size-2_5">{this.props.channelData.status}</h1>
          </div>
          <div className="game">
            <span className="user | bold">{this.props.userData.display_name}</span> playing <a className="game | bold rgb-text-40_40_40 no-underline" href={`http://www.twitch.tv/directory/game/${this.props.channelData.game}`}>{this.props.channelData.game}</a>
          </div>
        </div>
      </div>
      <div className="stream-section | pos-relative max-height-34_0 right-pad-22_0 y-marg-1_0">
        <div className="video-embed | full-width">
          <iframe className=" | fillout" src={`http://player.twitch.tv/?channel=${this.props.userData.name}`} frameBorder="0"></iframe>
        </div>
        <div className="chat-embed | pos-absolute pos-top-0_0 pos-right-0_0 width-21_0">
          <iframe className=" | fillout" src={`http://twitch.tv/${this.props.userData.name}/chat`} frameBorder="0"></iframe>
        </div>
      </div>
      <div className="options-belt |">
        <div className="auth-needed | col-2-1 left-justify">
          <div className="follow | inline-block x-marg-0_2 x-pad-0_3 y-pad-0_3 border-radius-0_3 rgb-bg-100_200_250 cursor-pointer">
            <span onClick={this.followChannel}>Follow</span>
        ...
        <div className="no-auth | col-2-1 right-justify">
          <div className="wraper | inline-block x-pad-0_5 y-pad-0_5 border-radius-0_3 rgb-bg-200_220_250">
            <div className="viewers | inline-block">
              <span className="counter | x-marg-0_5{(!this.props.streamData.stream) ? ' hidden' : ''}">{(this.props.streamData.stream) ? this.props.streamData.stream.viewers : 0}</span> <span>viewers</span>
            </div>
            <div className="views | inline-block">
              <span className="counter | x-marg-0_5">{this.props.channelData.views}</span> <span>views</span>
            </div>
            <div className="followers | inline-block">
              <span className="counter | x-marg-0_5">{this.props.channelData.followers}</span> <span>followers</span>
            </div>
          </div>
        </div>
      </div>
